...
public class TestSelfNotEquality extends MithraTestAbstract
{
    ...
    public void testIntegerNotEquality() throws Exception
    {
        ...
        OrderItemList shortList = new OrderItemList(OrderItemFinder.orderId().eq(2).and(
                OrderItemFinder.originalPrice().notEq(OrderItemFinder.discountPrice())));
    ...
    {
        OrderList list = new OrderList(OrderFinder.items().originalPrice().notEq(OrderFinder.items().discountPrice()));
