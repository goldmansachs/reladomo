...
public class MithraTestSequenceObjectFactoryForDefaultSource implements MithraSequenceObjectFactory
{
    public MithraSequence getMithraSequenceObject(String sequenceName, Object sourceAttribute, int initialValue)
    {
        MithraSequence mithraTestSequence = MithraTestSequenceForDefaultSourceFinder.findOne(MithraTestSequenceForDefaultSourceFinder.sequenceName().eq(sequenceName));

        if(mithraTestSequence == null)
        {
            mithraTestSequence = new MithraTestSequenceForDefaultSource();
            mithraTestSequence.setSequenceName(sequenceName);
            mithraTestSequence.setNextId(initialValue);
            ((MithraTestSequenceForDefaultSource)mithraTestSequence).insert();
        }
