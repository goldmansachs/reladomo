...
outer:
    ...
    public Timestamp getTimestampFromLong(long time)
    {
        int hash = HashUtil.hash(time);
        AtomicReferenceArray currentArray = getTable();
        while(true)
        {
            int length = currentArray.length();
            ...
            else if (o instanceof Entry)
            {
                ...
                while (e != null)
                {
                    Timestamp candidate = (Timestamp) e.get();
                    if (candidate != null &&
                            candidate.getTime() == time && candidate.getNanos() % 1000000 == 0)
                    {
                        return candidate;
                    }
                    e = e.getNext();
            ...
            {
                Timestamp candidate = (Timestamp) o;
                if (candidate.getTime() == time && candidate.getNanos() % 1000000 == 0)
                {
                    return candidate;
                }
            }
            return (Timestamp) getIfAbsentPut(factory.createTimestamp(time), true);
        }
    }
    ...
    private static class HardEntry implements Entry
    {
        ...
        public HardEntry(Object key, Entry next)
        {
            this.value = key;
            this.next = next;
        }
