...
public class RemoteMultiBatchUpdateResult extends MithraRemoteResult
{

    private MithraDataObject[] mithraDataObjects;
    private List updateWrappers;

    public RemoteMultiBatchUpdateResult()
    {
        // for externalizable
    }

    public RemoteMultiBatchUpdateResult(MithraDataObject[] mithraDataObjects, List updateWrappers)
    {
    ...
    public void run()
    {
        ...
        if (isDated)
        {
            for(int i=0;i<mithraDataObjects.length;i++)
            {
                ...
                else
                {
                    ...
                    for(int j=0;j<updateWrappers.size();j++)
                    {
                        AttributeUpdateWrapper uw = (AttributeUpdateWrapper) updateWrappers.get(j);
        ...
        else
        {
            for(int i=0;i<mithraDataObjects.length;i++)
            {
                ...
                mithraObject.zApplyUpdateWrappersForBatch(updateWrappers);
                for(int j=0;j<updateWrappers.size();j++)
                {
                    AttributeUpdateWrapper uw = (AttributeUpdateWrapper) updateWrappers.get(j);
                    tx.enrollAttribute(uw.getAttribute());
        ...
        MultiUpdateOperation multiUpdateOperation = new MultiUpdateOperation(this.updateWrappers, Arrays.asList(objectsToSetUpdated));
        mithraObjectPortal.getMithraObjectPersister().multiUpdate(multiUpdateOperation);
