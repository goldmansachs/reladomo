...
public class DetachedDeletedTxEnrollBehavior extends DetachedDeletedBehavior
{
    ...
    {
        if (mto.zEnrollInTransactionForRead(prevState, threadTx, PersistenceState.DETACHED_DELETED))
        {
    ...
    {
        if (mto.zEnrollInTransactionForWrite(prevState, new TransactionalState(threadTx, PersistenceState.DETACHED_DELETED)))
        {
            return AbstractTransactionalBehavior.getDetachedDeletedSameTxBehavior();
        }
        return null;
