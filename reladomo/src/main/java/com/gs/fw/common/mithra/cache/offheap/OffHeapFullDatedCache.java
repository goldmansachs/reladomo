...
import com.gs.fw.common.mithra.cache.LazyIndex;
...
public class OffHeapFullDatedCache extends AbstractDatedCache implements OffHeapSyncableCache
{
    ...
    public OffHeapFullDatedCache(Attribute[] nonDatedPkAttributes, AsOfAttribute[] asOfAttributes,
    ...
    {
        return new LazyIndex(new NonUniqueOffHeapIndex(extractors, 16, dataStorage));
    }
    ...
    {
        return new OffHeapSemiUniqueDatedIndex(extractors, asOfAttributes, dataStorage);
    }
    ...
    protected void reportSpaceUsage()
    {
        ...
        this.getSemiUniqueDatedIndex().reportSpaceUsage(this.getLogger(),  className);
        Index[] indexes = this.getIndices();
        for(int i=2;i<indexes.length;i++)
        {
            indexes[i].reportSpaceUsage(this.getLogger(), className);
        }
    }
