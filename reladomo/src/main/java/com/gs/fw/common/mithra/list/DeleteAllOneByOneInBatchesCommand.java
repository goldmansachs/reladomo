...
public class DeleteAllOneByOneInBatchesCommand implements TransactionalCommand, Synchronization
{
    ...
    public DeleteAllOneByOneInBatchesCommand(DelegatingList toBeDeleted, int[] batchSize)
    {
        this.toBeDeleted = toBeDeleted;
        this.batchSize = batchSize;
        toBeDeleted.setMaxObjectsToRetrieve(batchSize[0]);
    }
    ...
    {
        if (status != Status.STATUS_COMMITTED && status != Status.STATUS_COMMITTING && batchSize[0] > 100)
        {
            batchSize[0] /= 2;
        }
    }
