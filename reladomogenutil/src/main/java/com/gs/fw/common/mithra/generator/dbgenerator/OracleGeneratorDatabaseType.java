...
public class OracleGeneratorDatabaseType extends AbstractGeneratorDatabaseType
{
        ...
        for (int i = 0; i < indices.size(); i++)
        {
            ...
            if (index.isPk() && firstPk)
            {
                writer.println("alter table " + tableName + " add constraint "
                        + getFixedIndexName(index) + " primary key (" + index.getIndexColumns() + ")");
                firstPk = false;
            }
            else
            {
    ...
    private String getFixedIndexName(Index index)
    {
        ...
        if (name.length() > ORACLE_MAX_INDEX_NAME)
        {
            int targetLength = ORACLE_MAX_INDEX_NAME;
            ...
            if (idxIndex > 0)
            {
                ...
                targetLength = ORACLE_MAX_INDEX_NAME - (index.getName().length() - idxIndex);
            }
