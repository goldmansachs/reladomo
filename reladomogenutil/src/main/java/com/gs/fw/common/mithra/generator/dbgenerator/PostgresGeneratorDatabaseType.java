...
public class PostgresGeneratorDatabaseType extends AbstractGeneratorDatabaseType
{
    private static final int POSTGRES_MAX_INDEX_NAME = 63;
    ...
    {
        return com.gs.fw.common.mithra.generator.databasetype.PostgresDatabaseType.getInstance();
    }
    ...
    private String getFixedIndexName(Index index)
    {
        ...
        if (name.length() > POSTGRES_MAX_INDEX_NAME)
        {
            int targetLength = POSTGRES_MAX_INDEX_NAME;
            ...
            if (idxIndex > 0)
            {
                ...
                targetLength = POSTGRES_MAX_INDEX_NAME - (index.getName().length() - idxIndex);
            }
